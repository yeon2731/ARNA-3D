...
def preprocess(img, label_array, kidney_label_array):
    ...
    kidney_mask = (kidney_label_array == 2)
    fat_mask = (label_array == 6)
    tumor_mask = (label_array == 1)

    structure = np.ones((3,3,3), dtype=bool)
    dilated_kidney = binary_dilation(kidney_mask, structure=structure, iterations=3)
    kidney_boundary = dilated_kidney & ~kidney_mask
    kidney_boundary[tumor_mask] = False
